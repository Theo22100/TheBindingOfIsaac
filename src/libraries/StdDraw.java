...
//	SEE fonction rajouter
	 ...
	 * @param scaledHeight the height of the scaled image (in screen coordinates)
	 * @param degrees      is the number of degrees to rotate counterclockwise
	 * @param color        is the color you want to be accentuated
	 * @param transparency is the transparency wanted to add to the image
	 * @throws IllegalArgumentException if either {@code scaledWidth} or
	 *                                  {@code scaledHeight} is negative
	 * @throws IllegalArgumentException if the image filename is invalid
	 */
			...
			double degrees, Color color, int transparency)
	{
		...
		Color transparent = new Color(color.getRed(), color.getGreen(), color.getBlue(), transparency);
		BufferedImage secondImage;
		try {
			secondImage = ImageIO.read(new File(filename));
			for (int xx = 0; xx < secondImage.getWidth(); xx++) {
				for (int yy = 0; yy < secondImage.getHeight(); yy++) {
					Color c = new Color(secondImage.getRGB(xx, yy), true);
					if (c.getAlpha() == 255) secondImage.setRGB(xx, yy, transparent.getRGB());
					else secondImage.setRGB(xx, yy, c.getRGB());
				}
			}
			offscreen.drawImage(secondImage, (int) Math.round(xs - ws / 2.0), (int) Math.round(ys - hs /2.0), 
					(int) Math.round(ws),(int) Math.round(hs), null);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	/**
	 * clear the frame and disposes of it
	 * SEE fonction rajouter
	 */
...
//	SEE fonction rajouter
	/**
	 * Returns the first key from the KeysDown list, being pressed.
	 * <p>
	 * This method takes the keycode (corresponding to a physical key) as an
	 * argument. It can handle action keys (such as F1 and arrow keys) and modifier
	 * keys (such as shift and control). See {@link KeyEvent} for a description of
	...
	public static int keyPressed()
	{
		synchronized (keyLock)
		{
			if (keysDown.size() >= 1) return keysDown.first();
			return -1;
		}
	}
